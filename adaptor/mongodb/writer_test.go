...
func TestInsert(t *testing.T) {
	...
	c, _ := NewClient(WithURI(fmt.Sprintf("mongodb://transporter-mongo:27017/%s", writerTestData.DB)))
	s, err := c.Connect()
	if err != nil {
...
func TestRestartWrites(t *testing.T) {
	...
	c := &Client{
		uri:            fmt.Sprintf("mongodb://transporter-mongo:15000/%s", writerTestData.DB),
