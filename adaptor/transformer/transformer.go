...
func init() {
	adaptor.Add("transformer", func(p *pipe.Pipe, path string, extra adaptor.Config) (adaptor.Adaptor, error) {
		...
		if err != nil {
			return t, adaptor.Error{
				Lvl:    adaptor.CRITICAL,
				Path:   path,
				Err:    fmt.Sprintf("can't split transformer namespace (%s)", err.Error()),
				Record: nil,
			}
		}
...
func (t *Transformer) transformerError(lvl adaptor.ErrorLevel, err error, msg message.Msg) error {
	...
	if e, ok := err.(*otto.Error); ok {
		return adaptor.Error{
			Lvl:    lvl,
			Path:   t.path,
			Err:    fmt.Sprintf("transformer error (%s)", e.String()),
			Record: data,
		}
	}
	return adaptor.Error{
		Lvl:    lvl,
		Path:   t.path,
		Err:    fmt.Sprintf("transformer error (%s)", err.Error()),
		Record: data,
	}
}
