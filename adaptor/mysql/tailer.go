...
// Tailer implements the behavior defined by client.Tailer for interfacing with the MySQL binlog.
type Tailer struct {
...
// Use MySQL binlog to retrieve the latest changes
func (t *Tailer) pluckFromLogicalDecoding(s *Session, filterFn client.NsFilterFunc) ([]client.MessageSet, error) {
	...
	changesResult, err := s.mysqlSession.Query(fmt.Sprintf("SELECT * FROM pg_logical_slot_get_changes('%v', NULL, NULL);", t.replicationSlot))
	if err != nil {
		return result, err
	}
